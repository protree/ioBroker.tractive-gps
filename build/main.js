      ...
      for (const [key, value] of Object.entries(device)) {
        if (key === "latlong") {
          ...
          if (sysConfig && sysConfig.common && sysConfig.common.longitude && sysConfig.common.latitude) {
            const sysPoint = new import_geo_position.GeoPosition(sysConfig.common.latitude, sysConfig.common.longitude);
            const petPoint = new import_geo_position.GeoPosition(value[0], value[1]);
  ...
  async onMessage(obj) {
    if (typeof obj === "object" && obj.message) {
      if (obj.command === "refreshToken") {
        ...
        await this.getAccessToken();
        if (obj.callback)
          this.sendTo(obj.from, obj.command, "Successful", obj.callback);
      }
    }
  }
